...
The REST API call must by triggered by external code upon sensing a non-empty RabbitMQ queue via PEEK or other means. The REST API call will not return ary data and after about 10 seconds must be terminated by the client side; repeated attempts to start a new worker will be repeated while the REST API call is open - if docker-spaces is short of resources, keeping REST API call open longer increases the probability of a successful worker launch. Maxinum one worker will be launched per REST API call.
