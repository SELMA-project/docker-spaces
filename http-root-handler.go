...
type HTTPRootHostHandler struct {
	ID        string
	Target    *url.URL
	proxyHost *url.URL
}

func (h *HTTPRootHostHandler) String() string {
	return "HTTPRootHostHandler"
}

func (h *HTTPRootHostHandler) Closed(logger *ProxyLogger, request *ParsedHTTPRequest) {
...
func (h *HTTPRootHostHandler) resolveByReferrer(logger *ProxyLogger, request *ParsedHTTPRequest, referrer string) (targetAddress string, secure bool, err error) {
...
func (h *HTTPRootHostHandler) processRequestHead(logger *ProxyLogger, request *ParsedHTTPRequest, dryRun bool) (level int, targetAddress string, secure bool, err error) {

	// log := logger.WithExtension(": root-host: process-request-head")
	log := logger
	// fmt := log.E
	...
	if request.Path == "/" {
		// level = 0
		// targetAddress = h.TargetAddress
		targetAddress, secure, err = h.resolveByReferrer(logger, request, request.Path)
		// 	return
		// } else {
		// 	// NOTE: if referrer is a remote host, then assume root path is requested; in case of a container, a prefixed path (e.g., /container:.../ is required)
		//
		// 	// !!! IMPORTANT: this handler must be put last, so other root based handlers, like host and container handlers can trigger first
		//
	...
	// responds if:
	// 1. root /;
	// 2. /path without local referrer and cookie (if remote referrer, then cookie disabled);
	// 3. /path with local (no-host/same-host) / referrer
	// 4. /path with local (no-host/same-host) / cookie
	...
	if !dryRun && len(targetAddress) > 0 && len(h.Target.Path) > 0 {
		var newPath string
		newPath, err = URLJoinPath(h.Target.Path, request.Path)
		if err == nil {
			request.Path = newPath
		}
	}
...
func (h *HTTPRootHostHandler) RespondsAtLevel(logger *ProxyLogger, request *ParsedHTTPRequest) int {

	log := logger.WithExtension(": root-host: responds-at-level")
...
func (h *HTTPRootHostHandler) ProcessRequest(
	logger *ProxyLogger,
...
func (h *HTTPRootHostHandler) ProcessResponse(logger *ProxyLogger, response *ParsedHTTPResponse) (err error) {

	log := logger.WithExtension(": root-host: process-response")
	fmt := log.E
	...
	if response.StatusCode >= 300 && response.StatusCode < 400 {
		...
		if len(location) > 0 {
			...
			log.Tracef("got location URL: %#v", u)
			if err != nil {
				log.Debug("warning, unable to parse Location header value:", err)
			} else if u.Host == h.Target.Host {
				u.Host = h.proxyHost.Host
				u.Scheme = h.proxyHost.Scheme
				if len(h.Target.Path) > 0 && h.Target.Path != "/" {
					// strip prefix
					if strings.HasPrefix(u.Path, h.Target.Path) {
						u.Path = u.Path[len(h.Target.Path):]
					} else {
						log.Warn("response location has different prefix: %s, expected: %s", u.Path, h.Target.Path)
					}
				}
				response.Headers.Set("Location", u.String())
			}
			if false && len(u.Host) > 0 {
				// if u.Host != h.target
...
func (h *HTTPRootHostHandler) splitURLPath(path string) (prefix string, target string, targetPath string) {
